...
* If you are not still logged in, login into the {openshift_console_url}[OpenShift Console,window=_blank] using SSO with:
** username:
+
...
TIP: You could also find the url to this DevFile manually by opening your {gitea_console_url}[Gitea,window=_blank], logging in with your team's credentials and navigating to the repo `starter-app-python` and then `devfile.yaml`. Click on "Originalversion / Raw" and
copy the URL.
...
| `POST /led_on/<color>`
| Turns leds on, color can be red, green or blue
