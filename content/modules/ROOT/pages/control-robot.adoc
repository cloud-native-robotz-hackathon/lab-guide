...
* The Robot Control Page will now show a live camera view with detected objects
...
image::robot-control-page2-live.png[]
...
----
# Drop your code here
move_forward(application.config,10)
print('Done')
----
...
# Drop your code here
move_forward(application.config,10)
turn_left(application.config,90)
turn_right(application.config,180)
turn_left(application.config,90)
move_backward(application.config,10)
print('Done')
----
...
----
# Drop your code here
dist = distance(application.config)
...
----
# Drop your code here
objects = take_picture_and_detect_objects(application.config, class_labels)
...
That means you will need to create a loop of driving and checking the distance. Use the condition `+thread_event.is_set()+` in your while loop, so that the execution can be stopped from the **Stop** button command.
