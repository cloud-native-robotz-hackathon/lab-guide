...
Run the code with the **Run** button as usual.  Hold your hand in front of the sensor and see how tha value changes.  Pretty cool. No more bumping into stuff.
...
To enhance your development experience and see what the robot sees in the IDE, we provide two reference images with our example code that can be constantly replaced :
...
Here’s an example of how to arrange the IDE optimize your dev workflow:
...
That means you will need to create a loop of driving and checking the distance. Use the condition `+thread_event.is_set()+` in your while loop, so that you the execution can be stopped from the **Stop** button command.
