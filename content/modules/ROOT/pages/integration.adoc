...
Before we give you the full solution, want to try it yourself? 

**Can you write code that turns the robot toward a detected fedora?**

Hint: Calculate the center of the bounding box using `(coordinates.x_upper_left + coordinates.x_lower_right) / 2`. If the center is less than 320 (the middle of a 640px wide image), turn left. Otherwise, turn right.

Give it 2-3 minutes, then check out the MVP solution below!

== MVP (Minimal Viable Product)

To speed things up, here's a complete solution as an MVP.
...
🏆 **Mission accomplished!** Your robot found the hat! 
...
**Optimization Challenge:** Can you improve the stopping distance? Try adjusting the `delta` threshold value (currently 350) to make the robot stop closer or farther from the fedora.

== Feature Freeze

🎉 **Congratulations!** You now have a fully autonomous robot that can help you find your lost Fedora!
