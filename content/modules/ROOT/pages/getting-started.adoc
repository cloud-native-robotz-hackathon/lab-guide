...
=== Configure Your Robot Connection

Edit the file `config.py` to set up your robot connection:
...
TIP: Note that **Dev Spaces** saves your file directly while you edit it. No need to save manually. And as an added bonus gunicorn reloads your python app automatically, so there is also no need to restart your app.
...
The starter code provides helper functions to make coding easier:
...
* Write your code in the `startRobot()` function
* Click the **Initiate Run** button on the Robot Control Page
* Watch your robot execute your code!
* Use **Emergency Stop** if you need to stop execution

TIP: Sometimes the auto-reload of the app doesn't work and the app stops. If this happens:
* Click on the garbage can icon on the right of the terminal to stop the process
* Restart the app using the task: **Terminal** > **Run Task ...** > **devfile** > **devfile: Run the application**
...
* ✅ Robot connection configured
* ✅ Robot Control Page open
* ✅ Understanding of the Robot API

You're ready to tackle the coding challenges! Good luck! 🚀
