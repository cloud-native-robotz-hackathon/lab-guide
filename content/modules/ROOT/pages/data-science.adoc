...
There are a good number of tools available for this task, we'll use a well-known Open Source project called **Label Studio**. Label Studio runs in OpenShift, of course, and was already deployed for you. To access it, return to the OpenShift UI and choose the **Project** `{user}-label-studio`. On the overview page of the **Project**, from the **Inventory** section click the route and open it.

This will open the Label Studio UI, log in:
