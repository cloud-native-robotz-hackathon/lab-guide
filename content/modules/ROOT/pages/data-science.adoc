...
[NOTE]
====
Inferencing in AI is the process of using a trained model to make a prediction or generate a response based on new, live data. If training is the "studying" phase where the model learns patterns from a large dataset, inference is the "application" phase where it uses that knowledge to do its job.
====
...
This will show the running steps of the pipeline. Click on the steps to get more information like e.g. the logs.
...
🎉 **Excellent work!** You've successfully trained and deployed an AI model! 
...
Ready to build the app that will use this AI model to control the robot? Let's go!
