...
== Set Up Your Development Editor

To enhance your development experience, we provide two reference images with our example code:

- `static/current_view_box.jpg`
- `static/current_view.jpg`

Here’s an example of how to optimize your workflow:

video::devspaces-devenv.mp4[width=640]

== Coding Challenges

To become more familiar with the robot controls (and have some more fun), here are two challenges for you. Use your new robot skills and change the application to handle an obstacle win the challenges:
...
That means you will need to create loop of driving and checking the distance. Use the condition `+thread_event.is_set()+` in your while loop, so that you the execution can be stopped from the **Stop** button command.
